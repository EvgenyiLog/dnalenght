...
def analyze_single_spectrum(data: npt.NDArray[np.float64],
    ...
    heights, prominences, widths = [p[valid_mask] for p in (heights, prominences, widths)]
    
    if len(peaks) == 0:
        return np.array([]), np.array([]), {}
    
    # Скоринг
    scores = score_peaks(heights, prominences, widths, der2_flipped, peaks, first_reper_idx)
