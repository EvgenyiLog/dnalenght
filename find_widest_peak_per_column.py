...
def find_widest_peak_per_column(df: pd.DataFrame, order: int = 1, height_factor: float = 0.5) -> Dict[str, Dict]:
    ...
    for col in numeric_cols:
        ...
        for peak_idx, peak_val in peaks:
            ...
            # Левая граница пика
            left_bound = peak_idx
            while (left_bound > order
                   and series[left_bound] >= half_height):
                left_bound -= 1

            # Правая граница пика
            right_bound = peak_idx
            while (right_bound < len(series) - 1 - order
                   and series[right_bound] >= half_height):
                right_bound += 1
...
if __name__ == "__main__":
    ...
    signal1 = 2 * np.exp(-((t - 3) / 1.5)**2) + 1.2 * \
        np.exp(-((t - 3.2) / 0.4)**2) + 0.8 * np.exp(-((t - 2.8) / 0.3)**2)
